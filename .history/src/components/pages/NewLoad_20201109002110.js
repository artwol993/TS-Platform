...
function NewLoad({ onNewLoad }) {
    ...
    const handleNewLoad = (e) => {
        ...
        if (companyName.length < 2) {
            _errors.push("Nazwa firmy musi zaiwerać przynajmniej 2 znaki");
        }

        if (isNaN(shipmentWeight)) {
            _errors.push("Waga ładunku musi być liczbą")
        }

        if (isNaN(shipmentLength)) {
            _errors.push("Długość ładunku musi być liczbą")
        }

        if (userName.length < 2) {
            _errors.push("Nazwa firmy  zaiwerać przynajmniej 2 znaki");
        }

        if (userMail.length < 2 || email.indexOf("@")=== -1 ) {
